...
class aih_fer20250115Input : MLFeatureProvider {
...
@available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, visionOS 1.0, *)
class aih_fer20250115Output : MLFeatureProvider {
...
@available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, visionOS 1.0, *)
class aih_fer20250115 {
    let model: MLModel

    /// URL of model assuming it was installed in the same bundle as this class
    class var urlOfModelInThisBundle : URL {
        let bundle = Bundle(for: self)
        return bundle.url(forResource: "aih_fer20250115", withExtension:"mlmodelc")!
    }

    /**
        Construct aih_fer20250115 instance with an existing MLModel object.

        Usually the application does not use this initializer unless it makes a subclass of aih_fer20250115.
        Such application may want to use `MLModel(contentsOfURL:configuration:)` and `aih_fer20250115.urlOfModelInThisBundle` to create a MLModel object to pass-in.
    ...
    */
    ...
    /**
        Construct aih_fer20250115 instance with explicit path to mlmodelc file
        - parameters:
    ...
    */
    ...
    /**
        Construct aih_fer20250115 instance asynchronously with optional configuration.
    ...
    */
    class func load(configuration: MLModelConfiguration = MLModelConfiguration(), completionHandler handler: @escaping (Swift.Result<aih_fer20250115, Error>) -> Void) {
    ...
    /**
        ...
        - parameters:
          - configuration: the desired model configuration
    */
    class func load(configuration: MLModelConfiguration = MLModelConfiguration()) async throws -> aih_fer20250115 {
        try await load(contentsOf: self.urlOfModelInThisBundle, configuration: configuration)
    }

    /**
        Construct aih_fer20250115 instance asynchronously with URL of the .mlmodelc directory with optional configuration.
        ...
        - parameters:
          ...
          - handler: the completion handler to be called when the model loading completes successfully or unsuccessfully
    */
    class func load(contentsOf modelURL: URL, configuration: MLModelConfiguration = MLModelConfiguration(), completionHandler handler: @escaping (Swift.Result<aih_fer20250115, Error>) -> Void) {
        MLModel.load(contentsOf: modelURL, configuration: configuration) { result in
            switch result {
            case .failure(let error):
                handler(.failure(error))
            case .success(let model):
                handler(.success(aih_fer20250115(model: model)))
            }
        }
    }
    ...
    /**
        ...
        - parameters:
          ...
          - configuration: the desired model configuration
    */
    class func load(contentsOf modelURL: URL, configuration: MLModelConfiguration = MLModelConfiguration()) async throws -> aih_fer20250115 {
        let model = try await MLModel.load(contentsOf: modelURL, configuration: configuration)
        return aih_fer20250115(model: model)
    }
    ...
    /**
        ...
        - parameters:
           - input: the input to the prediction as aih_fer20250115Input

        - throws: an NSError object that describes the problem

        - returns: the result of the prediction as aih_fer20250115Output
    */
    func prediction(input: aih_fer20250115Input) throws -> aih_fer20250115Output {
    ...
    /**
        ...
        - returns: the result of the prediction as aih_fer20250115Output
    */
    func prediction(input: aih_fer20250115Input, options: MLPredictionOptions) throws -> aih_fer20250115Output {
        let outFeatures = try model.prediction(from: input, options: options)
        return aih_fer20250115Output(features: outFeatures)
    }
    ...
    */
    ...
    func prediction(input: aih_fer20250115Input, options: MLPredictionOptions = MLPredictionOptions()) async throws -> aih_fer20250115Output {
    ...
    /**
        ...
        - returns: the result of the prediction as aih_fer20250115Output
    */
    func prediction(input_leye: CVPixelBuffer, input_reye: CVPixelBuffer, input_face: CVPixelBuffer, input_mouth: CVPixelBuffer) throws -> aih_fer20250115Output {
        let input_ = aih_fer20250115Input(input_leye: input_leye, input_reye: input_reye, input_face: input_face, input_mouth: input_mouth)
    ...
    /**
        ...
        - parameters:
           - inputs: the inputs to the prediction as [aih_fer20250115Input]
           - options: prediction options
        ...
        - returns: the result of the prediction as [aih_fer20250115Output]
    */
    func predictions(inputs: [aih_fer20250115Input], options: MLPredictionOptions = MLPredictionOptions()) throws -> [aih_fer20250115Output] {
        let batchIn = MLArrayBatchProvider(array: inputs)
        let batchOut = try model.predictions(from: batchIn, options: options)
        var results : [aih_fer20250115Output] = []
        results.reserveCapacity(inputs.count)
        for i in 0..<batchOut.count {
            let outProvider = batchOut.features(at: i)
            let result =  aih_fer20250115Output(features: outProvider)
