...
void print_usage(const char* program) {
  std::cerr << "usage: " << program
            << " <manifest.json> <image.ppm> [encrypted_model_path] [model_id]"
            << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  const char* encrypted_model_path = argc >= 4 ? argv[3] : manifest_path;
  const char* model_id = argc >= 5 ? argv[4] : "aih_fer2025";
  ...
  if (status != EMOTION_STATUS_OK) {
    std::cerr << "emotion_init failed: " << status << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (status != EMOTION_STATUS_OK || unload_status != EMOTION_STATUS_OK ||
      shutdown_status != EMOTION_STATUS_OK) {
    std::cerr << "prediction failed: status=" << status
  ...
  std::cout << "top_label_index=" << result.top_label_index
