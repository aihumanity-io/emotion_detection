...
- Security decision record for Web model exposure. Started in
  `doc/web-model-protection-decision.md` with a no-go default for production
...
- Hosted inference fallback contract. Started in
  `doc/web-hosted-inference-contract.md` with browser-safe request/response
...
- Accuracy comparison against native ONNX baseline. Started in
  `doc/web-accuracy-baseline.md` with fixture, label, tolerance, and report
  field requirements for Web prototype comparisons.
- Bundle size and load-time report. Started in
  `doc/web-performance-baseline.md` with a checked-in Web source shell budget
  and prototype reporting requirements before generated release bundle reports
  are available.
- Browser smoke tests. Started in `doc/web-browser-smoke-harness.md` with
...
- Stable public release scope. Track current release readiness and platform
  support in `doc/release-readiness.md`.
